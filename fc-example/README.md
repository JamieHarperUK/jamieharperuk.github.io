# Custom Football Club Site
This directory is a plug-and-play custom football club website template, including an accompanying browser-based JSON editor for editing the site data via GitHub OAuth.
...
## Installation Steps
1 - Copy the contents of the `fc-template` directory into your now cloned GitHub Pages root directory folder.
...
export default {
  async fetch(request, env) {
    ...
    const siteTitle = String(env.SITE_TITLE || "Football Club");

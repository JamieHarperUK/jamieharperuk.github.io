...
const app = {
    ...
    analyticsId: "",
    siteMeta: {
        title: "Football Management Site",
        description: "Track my virtual team's, fixtures, and updates in one place.",
    ...
    getAnalyticsPagePath(path) {
        ...
        if (path === "posts") {
            return "/fm-template/?view=posts";
        }

        if (path === "hall-of-fame") {
            return "/fm-template/?view=hall-of-fame";
        }

        if (path.startsWith("post/")) {
            return `/fm-template/?view=post&slug=${encodeURIComponent(path.split("/")[1])}`;
        }

        if (path.startsWith("team/")) {
            return `/fm-template/?view=team&slug=${encodeURIComponent(path.split("/")[1])}`;
        }
